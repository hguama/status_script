...
def dibujar_punto_mouse(color):
    ...
    canvas_mouse.create_oval(
        0, 0, PUNTO_MOUSE, PUNTO_MOUSE,
        fill=color, outline=""
    )
...
# ---------- DETECCIÓN DE MOUSE SOBRE EL CÍRCULO SUPERIOR ----------
...
def check_cursor_encima():
    ...
    try:
        ...
        # Área del círculo
        left = pos_x
        right = pos_x + DIAMETRO
        top = pos_y
        bottom = pos_y + DIAMETRO

        # ¿El mouse está encima del círculo?
        encima = (left <= x <= right and top <= y <= bottom)
...
def main():
    ...
    root.after(20, check_cursor_encima)  # activar detección de proximidad
    root.mainloop()
